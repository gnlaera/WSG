...
function migrationLedgerHtml() {
  return `
    ...
    <p><strong>Status:</strong> Engine v2 0.1.6 is still not a replacement for Classic, but it now uses the Classic-style presentation layer as the target look and feel.</p>
...
function startWorker(mode) {
  try {
    ...
    if (mode === 'embedded') {
      ...
      worker = new Worker('./simulation-worker.js?v=0.1.6');
