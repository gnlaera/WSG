...
/* ============================================================
   ...
   Compacted for Engine v2 0.1.3.
   ============================================================ */

const BUILD_LABEL = 'WSG Engine v2 Build 0.1.3 - Viewport Fit Hotfix';
...
function createRenderer(canvas) {
  ...
  function calculateViewportFit(width, height) {
    const limiting = Math.max(1, Math.min(width, height));
    const padding = Math.max(18, limiting * 0.055);
    const safeHalf = Math.max(32, limiting * 0.5 - padding);
    const radius = Math.max(28, safeHalf / 1.05);
    return { cx: width * 0.5, cy: height * 0.5, radius };
  }
  ...
  function render(settings = {}) {
    ...
    const { cx, cy, radius } = calculateViewportFit(canvas.width, canvas.height);
