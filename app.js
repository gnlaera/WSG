...
function migrationLedgerHtml() {
  return `
    ...
    <p><strong>Status:</strong> Engine v2 0.1.8 restores Classic v1 intervention parity while preserving Worker-owned state and the compact three-file deployment.</p>
...
function bindPanelToggles(ui) {
  ...
  bindSimplePanelToggle(ui.toggleDiagnostics, ui.diagnosticsCard, 'Dev/Test');
  bindBottomPanelToggle(ui, ui.toggleLedger, ui.ledgerCard, 'Migration Ledger');
...
function startWorker(mode) {
  try {
    ...
    if (mode === 'embedded') {
      ...
      worker = new Worker('./simulation-worker.js?v=0.1.8');
      setStatus(ui, 'Worker: external simulation-worker.js starting.');
      addEvent(ui, 'External Worker attempted: simulation-worker.js?v=0.1.8.');
    }
...
addEvent(ui, 'Engine v2 main thread initialised. V1 intervention parity active. Deploy the three files together to GitHub Pages or another static host.');
