...
function toolIcon(toolId) {
  const icons = {
    ...
    mineral_seeding: '💎',
    seed_primitive_life: '🧬',
    planetary_stabilisation: '✨',
    seed_early_settlers: '🏕️'
  };
  return icons[toolId] || '•';
}
...
function migrationLedgerHtml() {
  return `
    ...
    <p><strong>Status:</strong> Engine v2 0.1.7 is still not a replacement for Classic, but it now uses the Classic-style presentation layer as the target look and feel.</p>
    <div class="ledger-grid">
      ...
      <section><h4>Intentionally deferred</h4><p>Full Classic parity, full Earth matching, all Classic panels, save/load, scenarios, advanced graphics backends, imported data, and asset bundles.</p></section>
...
function startWorker(mode) {
  try {
    ...
    if (mode === 'embedded') {
      ...
      worker = new Worker('./simulation-worker.js?v=0.1.7');
      setStatus(ui, 'Worker: external simulation-worker.js starting.');
      addEvent(ui, 'External Worker attempted: simulation-worker.js?v=0.1.7.');
    }
...
addEvent(ui, 'Engine v2 main thread initialised. Classic parity functional hotfix active. Deploy the three files together to GitHub Pages or another static host.');
